...
def pop_vectors_from_doc(doc_source):
    """
    Remove the dense_vector fields from a document's _source before returning it.
    """
    ...
    try:
        ...
        for doc_id, combined_score in ranked_matches:
            try:
                ...
                doc.pop("academic_vec", None)
                doc.pop("professional_vec", None)
                doc.pop("mentorship_topic_vec", None)
...
# ─── NEW /recommend ENDPOINT (for “normal_user” fallback only) ─────────────────
